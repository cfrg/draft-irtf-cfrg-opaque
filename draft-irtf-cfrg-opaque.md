...
- DeserializeElement(buf): Attempt to map input byte array `buf` to an OPRF group element.
  This function can raise a DeserializeError upon failure; see Section 2.1 of {{RFC9497}}
  for more details.
...
- DeriveDiffieHellmanKeyPair(seed): This function is implemented as
  ...
  as specified in Section 3.2 of {{RFC9497}}. The public value from DeriveKeyPair
...
- DiffieHellman(k, B): Implemented as scalar multiplication as described in
  ...
  group described in Section 2.1 of {{RFC9497}}.
...
- The OPRF protocol uses the `modeOPRF` configuration of Section 3.1 of {{RFC9497}} and
...
- {{JKX18}} used a generic prime-order group for the DH-OPRF and HMQV operations,
  ...
  Section 2.1 of {{RFC9497}}. This specification also delegates OPRF group
  choice and operations to Section 4 of {{!RFC9497}}. As such, the prime-order group as used
...
- {{JKX18}} specified DH-OPRF (see Appendix B) to instantiate
  ...
  instantiates the OPRF functionality based on Section 3.3.1 of {{RFC9497}}, which
...
see Section 7.2.3 of {{RFC9497}} for more information. For OPAQUE, these attacks
